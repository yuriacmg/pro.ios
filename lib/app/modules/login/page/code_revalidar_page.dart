...
class CodeRevalidarPage extends StatefulWidget {
  CodeRevalidarPage({required this.recodeSend, super.key});
  RecodeAccountEmailSendModel recodeSend;

  @override
  State<CodeRevalidarPage> createState() => _CodeRevalidarPageState();
}

class _CodeRevalidarPageState extends State<CodeRevalidarPage> {
          ...
          children: [
                            ...
                            Row(
                              children: [
                                Padding(
                                  ...
                                  child: Text(
                                    hiddenCorreo(widget.recodeSend.vCorreo!),
                                ...
                                children: [
                                  TextButton(
                                    onPressed: !newCode
                                        ...
                                        : () {
                                            //reenviar codigo
                                           context.read<LoginBloc>().add(RecodeEmailEvent(widget.recodeSend, true));
                                          },
                                    child: Text(
                                      'Reenviar código',
                                      style: TextStyle(
                                    ...
                                    BottonCenterWidget(
                                      ...
                                      ontap: state is CodeChangeState && state.code.length == 6
                                          ? () {
                                              ...
                                              final send = ValidCodeEmailSendModel(
                                                uiDTransacion:'',
                                                vCodigo: state.code,
                                                vCorreo: widget.recodeSend.vCorreo,
                                              );

                                              context.read<LoginBloc>().add(ValidCodeEmailEvent(send));
                                            }
                                          : () {},
                                      text: 'Aceptar',
                                    ),
            ...
            BlocListener<LoginBloc, LoginState>(
              listener: (context, state) {
                ...
                if (state is ValidCodeEmailSuccessState) {
                ...
                if (state is RecodeAccountEmailSuccessState) {
                  context.read<LoaderCubit>().hideLoader();
                    if(state.isResend){
                      final alert = DialogInformWidget(
                      title: 'Información',
                ...
                if (state is ErrorValidCodeEmailState) {
                  context.read<LoaderCubit>().hideLoader();
                  context.read<CodeBloc>().add(CodeCleanEvent());
